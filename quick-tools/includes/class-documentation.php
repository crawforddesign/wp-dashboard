...
class Quick_Tools_Documentation {
    ...
    /**
     * Available permission levels for documentation.
     */
    ...
    /**
     * Add permissions meta box to documentation edit screen.
     */
    public function add_permissions_meta_box() {
        add_meta_box(
            'qt_documentation_permissions',
            __('Documentation Permissions', 'quick-tools'),
    ...
    /**
     * Render the permissions meta box.
     */
    public function render_permissions_meta_box($post) {
        // Add nonce for security
        wp_nonce_field('qt_documentation_permissions', 'qt_documentation_permissions_nonce');

        // Get current permission level
        $current_permission = get_post_meta($post->ID, '_qt_permission_level', true);
        
        // Default to administrators only if not set
        if (empty($current_permission)) {
            $current_permission = 'manage_options';
        }

        ?>
        <div class="qt-permissions-meta-box">
            <p><strong><?php _e('Who can view this documentation?', 'quick-tools'); ?></strong></p>
            
            <?php foreach (self::PERMISSION_LEVELS as $capability => $label) : ?>
                <label style="display: block; margin-bottom: 8px;">
                    <input type="radio" 
                           name="qt_permission_level" 
                           value="<?php echo esc_attr($capability); ?>" 
                           <?php checked($current_permission, $capability); ?>>
                    <?php echo esc_html($label); ?>
                </label>
            <?php endforeach; ?>
            
            <div style="margin-top: 15px; padding: 10px; background: #f8f9fa; border-left: 4px solid #2271b1;">
                <p style="margin: 0; font-size: 12px; color: #666;">
                    <strong><?php _e('Permission Levels:', 'quick-tools'); ?></strong><br>
                    • <strong><?php _e('Administrators Only:', 'quick-tools'); ?></strong> <?php _e('Only site administrators', 'quick-tools'); ?><br>
                    • <strong><?php _e('Editors and Above:', 'quick-tools'); ?></strong> <?php _e('Editors and administrators', 'quick-tools'); ?><br>
                    • <strong><?php _e('Authors and Above:', 'quick-tools'); ?></strong> <?php _e('Authors, editors, and administrators', 'quick-tools'); ?><br>
                    • <strong><?php _e('All Logged-in Users:', 'quick-tools'); ?></strong> <?php _e('Any user who can log into WordPress', 'quick-tools'); ?>
                </p>
            </div>
        </div>
        <?php
    ...
    /**
     * Save permissions meta box data.
     */
    public function save_permissions_meta_box($post_id) {
        // Security checks
        if (!isset($_POST['qt_documentation_permissions_nonce'])) {
            return;
        }
        
        if (!wp_verify_nonce($_POST['qt_documentation_permissions_nonce'], 'qt_documentation_permissions')) {
            return;
        }
        ...
        if (isset($_POST['qt_permission_level'])) {
            $permission_level = sanitize_text_field($_POST['qt_permission_level']);
            
            // Validate against our allowed permission levels
            if (array_key_exists($permission_level, self::PERMISSION_LEVELS)) {
                update_post_meta($post_id, '_qt_permission_level', $permission_level);
            }
        }
    }
    ...
    public function add_dashboard_widgets() {
        // Check if any user can view documentation (not just admins anymore)
        ...
        foreach ($categories as $category) {
            // Check if user can view any documentation in this category
            if ($this->user_can_view_any_in_category($category->term_id)) {
    ...
    /**
     * Check if user can view any documentation in a category.
     */
    private function user_can_view_any_in_category($category_id) {
        ...
        foreach ($docs as $doc_id) {
            if ($this->user_can_view_documentation($doc_id)) {
                return true;
            }
        }
    ...
    public function render_dashboard_widget($post, $callback_args) {
        ...
        $docs = get_posts(array(
            'post_type' => self::POST_TYPE,
            'post_status' => 'publish',
            'numberposts' => $max_items * 2, // Get more to account for permission filtering
        ...
        // Filter docs by permissions and limit results
        $viewable_docs = array();
        foreach ($docs as $doc) {
            if ($this->user_can_view_documentation($doc->ID)) {
                $viewable_docs[] = $doc;
                if (count($viewable_docs) >= $max_items) {
                    break;
                }
            }
        }

        if (empty($viewable_docs)) {
            echo '<p>' . sprintf(__('No documentation found in %s category that you have permission to view.', 'quick-tools'), $category->name) . '</p>';
            
            // Only show "Add Documentation" link to users who can create documentation
        ...
        foreach ($viewable_docs as $doc) {
            ...
            $permission_label = $this->get_permission_label($doc->ID);
            ...
            echo '<span class="qt-doc-permission" title="' . __('Who can view this', 'quick-tools') . '">' . esc_html($permission_label) . '</span>';
            
            // Only show edit link to users who can edit
    ...
    public function render_documentation_viewer() {
        ...
        // Check if user has permission to view this documentation
        if (!$this->user_can_view_documentation($post->ID)) {
            wp_die(__('You do not have permission to view this documentation.', 'quick-tools'));
        }
        ...
        $permission_label = $this->get_permission_label($post->ID);
            ...
            <div class="qt-doc-header">
                ...
                <div class="qt-doc-meta">
                    <div class="qt-doc-info">
                        ...
                        <span class="qt-doc-permission-info">
                            <strong><?php _e('Visible to:', 'quick-tools'); ?></strong> <?php echo esc_html($permission_label); ?>
                        </span>
                        <span class="qt-doc-views">
                            <strong><?php _e('Views:', 'quick-tools'); ?></strong> <?php echo number_format_i18n($view_count); ?>
                        </span>
                    </div>
    ...
    public function search_documentation($search_term) {
        ...
        $docs = get_posts(array(
            'post_type' => self::POST_TYPE,
            'post_status' => 'publish',
            'numberposts' => 50, // Get more results to filter by permissions
            's' => $search_term,
            'orderby' => 'relevance',
        ));
        ...
        $results = array();
        foreach ($docs as $doc) {
            // Only include docs the current user can view
            if (!$this->user_can_view_documentation($doc->ID)) {
                continue;
            }
            ...
            $results[] = array(
                ...
                'permission_level' => $this->get_permission_label($doc->ID),
                'view_url' => admin_url('admin.php?page=qt-view-documentation&post_id=' . $doc->ID),
                'edit_url' => current_user_can('edit_post', $doc->ID) ? admin_url('post.php?post=' . $doc->ID . '&action=edit') : '',
            );
